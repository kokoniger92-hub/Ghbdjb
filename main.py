...
DATABASE_PATH = os.getenv('DATABASE_PATH', '/tmp/proxy_bot.db')
...
class Database:
    ...
    async def init(self):
        async with aiosqlite.connect(self.path) as db:
            ...
            default_countries = [
                ...
                'Бразилия', 'Индия', 'Австралия', 'Сингапур', 'Турция'
            ]
    ...
    async def toggle_ban(self, user_id):
        user = await self.get_user(user_id)
        if user:
            new_status = 0 if user['is_banned'] else 1
            await self.execute(
                'UPDATE users SET is_banned = ? WHERE user_id = ?',
                [new_status, user_id], fetch=False
            )
            return new_status
        return None
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id in ADMIN_IDS:
        admin_url = f"{WEBHOOK_URL}/admin" if WEBHOOK_URL else "недоступна"
        await update.message.reply_text(
            f"🔧 *Админ-панель*\n\n{welcome}\n\n"
            f"🌐 Веб-админка: {admin_url}\n"
...
async def handle_state_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get('action') == 'awaiting_quantity':
        try:
            ...
            await update.message.reply_text(
                f"📦 {state['proxy_type'].upper()} × {qty}\n\n"
                f"💵 Базовая цена: ${float(await db.get_setting(f'base_price_{state[\"proxy_type\"]}', '2.0')):.2f}/день\n\n"
                f"🌍 Выберите страну:",
    ...
    elif state.get('action') == 'awaiting_country_input':
        country_name = text
        state['country'] = country_name
        state['action'] = 'awaiting_duration'
...
async def handle_admin_menu(update: Update, context, text):
    ...
    if text == "📋 Заявки":
        ...
        for u in users[:20]:
            ...
            msg += (
                f"{ban} {u['first_name'] or 'Без имени'}"
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("type_"):
        ...
        await query.edit_message_text(
            f"📦 *{proxy_type.upper()}*\n\n"
            f"💵 Базовая цена: ${float(await db.get_setting(f'base_price_{proxy_type}', '2.0')):.2f}/день\n\n"
...
async def lifespan(app: FastAPI):
    ...
    if WEBHOOK_URL and telegram_app:
...
async def webhook_info():
    ...
    return {"url": webhook.url, "pending_updates": webhook.pending_update_count}
...
<head>
    ...
    <title>Proxy Bot - Админ-панель</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status { padding: 4px 12px; border-radius: 20px; font-size: 13px; font-weight: 600; }
...
<body>
    <div class="container">
        <div class="login" id="loginBox">
            ...
            <input type="password" id="password" placeholder="Пароль администратора" onkeypress="if(event.key==='Enter')login()">
        ...
        <div class="dashboard" id="dashboard">
            <div class="header">
                ...
                <button class="refresh-btn" onclick="loadAll()">🔄 Обновить данные</button>
            </div>
    ...
    <script>
        ...
        async function api(url) {
            const res = await fetch(url + (url.includes('?') ? '&' : '?') + 'password=' + password);
            if (res.status === 403) { alert('Неверный пароль'); location.reload(); return null; }
            return res.json();
        }
        
        async function loadAll() {
            const [stats, orders, topups] = await Promise.all([
                api('/api/stats'),
                api('/api/orders'),
                api('/api/topups')
            ]);
            ...
            if (stats) {
                document.getElementById('stats').innerHTML = `
                    ...
                    <div class="stat-card"><h3>Всего заказов</h3><div class="value">${stats.total_orders}</div></div>
        ...
        function renderOrders(orders) {
            if (!orders || orders.length === 0) {
                document.getElementById('orders').innerHTML = '<div class="empty">✅ Нет новых заявок</div>';
                return;
            }
            
            let html = `<table><thead><tr>
                <th>ID</th><th>Пользователь</th><th>Тип</th><th>Кол-во</th>
                <th>Страна</th><th>Срок</th><th>Цена</th><th>Дата</th><th>Действия</th>
            </tr></thead><tbody>`;
            
            orders.forEach(o => {
                html += `<tr>
                    <td><b>#${o.id}</b></td>
                    <td>${o.first_name}<br><small>@${o.username}</small></td>
                    <td>${o.proxy_type}</td>
                    <td>${o.quantity}</td>
                    <td>${o.country}</td>
                    <td>${o.duration}</td>
                    <td><b>$${o.price}</b></td>
                    <td><small>${o.created_at?.substring(0, 16)}</small></td>
                    <td>
                        <button class="btn btn-success" onclick="updateOrder(${o.id},'in_progress')">✅</button>
                        <button class="btn btn-danger" onclick="updateOrder(${o.id},'rejected')">❌</button>
                    </td>
                </tr>`;
            });
        ...
        function renderTopups(topups) {
            if (!topups || topups.length === 0) {
                document.getElementById('topups').innerHTML = '<div class="empty">✅ Нет ожидающих пополнений</div>';
                return;
            }
            
            let html = `<table><thead><tr>
                <th>ID</th><th>Пользователь</th><th>Сумма</th><th>Дата</th><th>Действия</th>
            </tr></thead><tbody>`;
            
            topups.forEach(t => {
                html += `<tr>
                    <td><b>#${t.id}</b></td>
                    <td>${t.first_name}<br><small>@${t.username}</small></td>
                    <td><b>$${t.amount}</b></td>
                    <td><small>${t.created_at?.substring(0, 16)}</small></td>
                    <td>
                        <button class="btn btn-success" onclick="approveTopup(${t.id})">✅</button>
                        <button class="btn btn-danger" onclick="rejectTopup(${t.id})">❌</button>
                    </td>
                </tr>`;
            });
        ...
        async function updateOrder(id, status) {
            ...
            await fetch(`/api/order/${id}/status`, { method: 'POST', body: form });
            loadAll();
        }
        ...
        async function approveTopup(id) {
            const form = new FormData();
            form.append('password', password);
            await fetch(`/api/topup/${id}/approve`, { method: 'POST', body: form });
            loadAll();
        }
        ...
        async function rejectTopup(id) {
            const form = new FormData();
            form.append('password', password);
            await fetch(`/api/topup/${id}/reject`, { method: 'POST', body: form });
            loadAll();
        }
    </script>
</body>
