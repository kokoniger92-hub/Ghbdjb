...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id in ADMIN_IDS:
        ...
        await update.message.reply_text(
            f"🔧 *Админ-панель*\n\n{welcome}\n\n👥 Пользователи заходят через Mini App\n📢 Ты можешь делать рассылку\n💬 Чат с клиентами внутри бота",
...
async def handle_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 Выйти из чата":
        ...
        await update.message.reply_text("Вы вышли из чата.", reply_markup=ADMIN_KEYBOARD); return
...
async def handle_state_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔙 Отмена":
        ...
        kb = ADMIN_KEYBOARD if user.id in ADMIN_IDS else ReplyKeyboardMarkup([
            [KeyboardButton("🛒 Открыть магазин", web_app=WebAppInfo(url=f"{APP_URL}/app"))],
            [KeyboardButton("📋 Мои заказы"), KeyboardButton("💳 Баланс")],
    ...
    if state.get('action') == 'awaiting_topup':
        try:
            ...
            await update.message.reply_text(f"✅ Заявка #{topup_id} на ${amount:.2f} создана!\n⏳ Ожидайте подтверждения.",
                reply_markup=ReplyKeyboardMarkup([[KeyboardButton("🛒 Открыть магазин", web_app=WebAppInfo(url=f"{APP_URL}/app"))], [KeyboardButton("📋 Мои заказы"), KeyboardButton("💳 Баланс")]], resize_keyboard=True))
        ...
        success = sum(1 for uid in users if not (await asyncio.sleep(0.05)) and not (lambda: None)())
        for uid in users:
        ...
        try:
            ...
            history = f"💬 *Чат по заказу #{oid}*\n\n"
            for m in messages[-20:]:
                sender = "Вы" if m['from_admin'] else "Клиент"
                history += f"*{sender}*: {m['message']}\n"
            history += "\n_Введите сообщение для клиента_"
            del user_states[user.id]
            await update.message.reply_text(history, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Выйти из чата", callback_data="close_chat")]]))
        except ValueError: await update.message.reply_text("❌ Введите ID заказа (число):")
    
    elif state.get('action') == 'awaiting_mirror_name':
        state['mirror_name'] = text; state['action'] = 'awaiting_mirror_token'
        await update.message.reply_text("🔑 Введите токен бота-зеркала (от @BotFather):")
    
    elif state.get('action') == 'awaiting_mirror_token':
        state['mirror_token'] = text; state['action'] = 'awaiting_mirror_url'
        await update.message.reply_text("🔗 Введите URL зеркала (например: https://bot2.onrender.com):")
    
    elif state.get('action') == 'awaiting_mirror_url':
        await db.add_mirror(state['mirror_name'], state['mirror_token'], text); del user_states[user.id]
        await update.message.reply_text(f"✅ Зеркало *{state['mirror_name']}* добавлено!\n\nURL: {text}", reply_markup=ADMIN_KEYBOARD, parse_mode=ParseMode.MARKDOWN)

async def handle_admin_menu(update: Update, context, text):
    user = update.effective_user
    
    if "Пополнения" in text or "пополнения" in text.lower():
        topups = await db.get_pending_topups()
        if not topups: await update.message.reply_text("✅ Нет ожидающих пополнений\n\nСоздайте заявку: кнопка 💰 Пополнить"); return
        for t in topups:
            ui = await db.get_user(t['user_id']); bal = ui['balance'] if ui else 0
            msg = f"💰 *Пополнение #{t['id']}*\n\n👤 {t['first_name']} (@{t['username']})\n💵 Сумма: ${t['amount']:.2f}\n💳 Баланс клиента: ${bal:.2f}\n📅 {t['created_at'][:19]}"
    ...
    elif "Новые заявки" in text or "заявки" in text.lower():
        orders = await db.get_pending_orders()
        if not orders: await update.message.reply_text("✅ Нет новых заявок"); return
        for o in orders:
            msg = f"📋 *Заявка #{o['id']}*\n\n👤 {o['first_name']} (@{o['username']})\n🆔 `{o['user_id']}`\n📦 {o['proxy_type'].upper()} × {o['quantity']}\n🌍 {o['country']} | ⏱ {o['duration']}\n💰 ${o['price']:.2f}\n📅 {o['created_at'][:19]}"
            kb = [[InlineKeyboardButton("✅ Взять в работу", callback_data=f"take_{o['id']}"), InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{o['id']}")]]
            await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
        return
    
    elif "Заявки в работе" in text or "в работе" in text.lower():
        orders = await db.get_in_progress_orders()
        if not orders: await update.message.reply_text("✅ Нет заявок в работе"); return
        for o in orders:
            msg = f"🔄 *Заявка #{o['id']}*\n\n👤 {o['first_name']} (@{o['username']})\n📦 {o['proxy_type'].upper()} × {o['quantity']}\n🌍 {o['country']} | ⏱ {o['duration']}\n💰 ${o['price']:.2f}"
    ...
    elif "Чат" in text or "чат" in text.lower():
        user_states[user.id] = {'action': 'awaiting_chat_order'}
        await update.message.reply_text("Введите ID заказа для открытия чата:", reply_markup=CANCEL_KEYBOARD); return
    
    elif "Статистика" in text or "статистика" in text.lower():
    ...
    elif "Пользователи" in text or "пользователи" in text.lower():
        users = await db.get_all_users()
        msg = "👥 *Пользователи:*\n\n"
        for u in users[:30]:
            ban = "🚫" if u['is_banned'] else "✅"; uname = f" (@{u['username']})" if u['username'] else ""
            msg += f"{ban} {u['first_name'] or '—'}{uname} | 💳 ${u['balance']:.2f} | ID: `{u['user_id']}`\n"
        await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN); return
    
    elif "Рассылка" in text or "рассылка" in text.lower():
        user_states[user.id] = {'action': 'awaiting_broadcast'}
        await update.message.reply_text("📢 Введите текст для рассылки всем пользователям:", reply_markup=CANCEL_KEYBOARD); return
    
    elif "Настройки" in text or "настройки" in text.lower():
        enabled = await db.get_setting('orders_enabled'); status = "✅" if enabled == 'true' else "❌"
        prices = {'http': await db.get_setting('base_price_http','2.0'), 'socks5': await db.get_setting('base_price_socks5','2.5'), 'residential': await db.get_setting('base_price_residential','5.0'), 'datacenter': await db.get_setting('base_price_datacenter','1.5')}
        msg = f"⚙️ *Настройки*\n\nПрием заказов: {status}\n\n*Цены за день:*\n• HTTP: ${prices['http']}\n• SOCKS5: ${prices['socks5']}\n• Residential: ${prices['residential']}\n• Datacenter: ${prices['datacenter']}"
        kb = [[InlineKeyboardButton("🔄 Заказы", callback_data="toggle_orders")], [InlineKeyboardButton("✏️ Приветствие", callback_data="edit_welcome")], [InlineKeyboardButton("💵 HTTP", callback_data="setprice_http"), InlineKeyboardButton("💵 SOCKS5", callback_data="setprice_socks5")], [InlineKeyboardButton("💵 Residential", callback_data="setprice_residential"), InlineKeyboardButton("💵 Datacenter", callback_data="setprice_datacenter")]]
        await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN); return
    
    elif "Страны" in text or "страны" in text.lower():
    ...
    elif "Зеркала" in text or "зеркала" in text.lower():
        mirrors = await db.get_mirrors()
        if not mirrors: msg = "🔗 *Зеркала ботов*\n\nПока нет зеркал."
        else:
            msg = "🔗 *Зеркала ботов:*\n\n"
            for m in mirrors:
                status = "✅" if m['is_active'] else "❌"
                msg += f"{status} *{m['name']}*\n   URL: `{m['url']}`\n   ID: {m['id']}\n\n"
        kb = [[InlineKeyboardButton("➕ Добавить зеркало", callback_data="add_mirror")], [InlineKeyboardButton("🔄 Переключить", callback_data="toggle_mirror_menu")], [InlineKeyboardButton("❌ Удалить", callback_data="delete_mirror_menu")]]
        await update.message.reply_text(msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN); return
...
async def handle_user_menu(update: Update, context, text):
    ...
    if text == "📋 Мои заказы":
        ...
        ud = await db.get_user(user.id); bal = ud['balance'] if ud else 0
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; user = query.from_user; data = query.data; await query.answer()
    ...
    if data.startswith("take_"):
        oid = int(data.split("_")[1]); await db.update_order_status(oid, 'in_progress', user.id)
        order = await db.get_order(oid)
        ...
        oid = int(data.split("_")[1]); order = await db.get_order(oid)
        if order:
        ...
        oid = int(data.split("_")[1]); user_states[user.id] = {'action': 'awaiting_proxy_data', 'order_id': oid}
        await query.edit_message_text(f"{query.message.text}\n\n📝 *Введите данные прокси для отправки клиенту:*", parse_mode=ParseMode.MARKDOWN)
    
    elif data.startswith("chat_"):
        oid = int(data.split("_")[1]); active_chats[user.id] = oid
        order = await db.get_order(oid); messages = await db.get_chat_messages(oid)
        history = f"💬 *Чат по заказу #{oid}*\n\n"
        for m in messages[-20:]:
            sender = "Вы" if m['from_admin'] else "Клиент"
            history += f"*{sender}*: {m['message']}\n"
        history += "\n_Введите сообщение для клиента_"
        await query.edit_message_text(history, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Выйти из чата", callback_data="close_chat")]]))
        ...
        oid = int(data.split("_")[1]); success, result = await db.cancel_order(oid, user.id)
        ...
        tid = int(data.split("_")[2]); topup = await db.approve_topup(tid, user.id)
        if topup:
        ...
        tid = int(data.split("_")[2]); topup = await db.reject_topup(tid, user.id)
        if topup:
        ...
        user_states[user.id] = {'action': 'awaiting_welcome'}; await query.edit_message_text("✏️ Введите приветствие:")
    
    elif data.startswith("setprice_"):
        pt = data.split("_")[1]; user_states[user.id] = {'action': 'awaiting_price', 'type': pt}
        await query.edit_message_text(f"💵 Цена для {pt.upper()}:")
    
    elif data == "add_country":
        user_states[user.id] = {'action': 'awaiting_new_country'}; await query.edit_message_text("🌍 Название страны:")
    
    elif data == "remove_country":
        user_states[user.id] = {'action': 'awaiting_remove_country'}; await query.edit_message_text("🌍 Страна для удаления:")
    
    elif data == "add_mirror":
        user_states[user.id] = {'action': 'awaiting_mirror_name'}; await query.edit_message_text("🔗 Введите название зеркала:")
        ...
        await query.edit_message_text("Выберите зеркало для переключения:", reply_markup=InlineKeyboardMarkup(kb))
    
    elif data == "delete_mirror_menu":
        mirrors = await db.get_mirrors()
        if not mirrors: await query.edit_message_text("Нет зеркал"); return
        kb = [[InlineKeyboardButton(f"❌ {m['name']}", callback_data=f"delete_mirror_{m['id']}")] for m in mirrors]
        await query.edit_message_text("Выберите зеркало для удаления:", reply_markup=InlineKeyboardMarkup(kb))
    
    elif data.startswith("toggle_mirror_"):
        mid = int(data.split("_")[2]); status = await db.toggle_mirror(mid)
        await query.edit_message_text(f"🔗 Статус изменен на {'✅ Активен' if status else '❌ Неактивен'}")
    
    elif data.startswith("delete_mirror_"):
        mid = int(data.split("_")[2]); await db.delete_mirror(mid)
...
async def user_app():
    html = """
<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no"><title>Магазин прокси</title>
...
.star{position:absolute;color:#fff;opacity:0;animation:fall linear infinite;font-size:8px;text-shadow:0 0 4px rgba(255,255,255,0.5)}
@keyframes fall{0%{transform:translateY(-20px) translateX(0);opacity:0}5%{opacity:0.8}50%{opacity:0.5}100%{transform:translateY(105vh) translateX(30px);opacity:0}}
...
.card{background:#111;border:1px solid #333;padding:20px;border-radius:16px;margin-bottom:14px;transition:border-color 0.2s}
.card h2{font-size:16px;font-weight:600;margin-bottom:14px;color:#fff}
.btn{display:block;width:100%;padding:14px;margin:6px 0;background:#fff;color:#000;border:none;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;transition:all 0.2s;letter-spacing:-0.3px}
.btn:active{background:#ccc;transform:scale(0.98)}
.balance{font-size:36px;font-weight:700;text-align:center;color:#fff;margin:16px 0;letter-spacing:-1px}
.price-display{text-align:center;margin:16px 0;font-size:18px;font-weight:600;color:#fff}
.price-display b{font-size:22px;letter-spacing:-0.5px}
.order-card{background:#111;border:1px solid #333;padding:16px;border-radius:12px;margin-bottom:10px}
.status{display:inline-block;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:600;letter-spacing:0.5px}
.status-pending{background:#fff;color:#000}
.status-in_progress{background:#333;color:#fff}
.status-completed{background:#fff;color:#000;border:1px solid #fff}
.status-cancelled,.status-rejected{background:transparent;border:1px solid #555;color:#999}
input,select{width:100%;padding:12px 14px;margin:6px 0;background:#0a0a0a;border:1px solid #333;border-radius:10px;color:#fff;font-size:15px;font-family:inherit;transition:border-color 0.2s;-webkit-appearance:none;appearance:none}
...
.tab{flex:1;padding:11px;text-align:center;border-radius:10px;cursor:pointer;font-weight:600;font-size:14px;color:#999;transition:all 0.2s}
...
.proxy-data{margin-top:10px;padding:10px;background:#0a0a0a;border:1px solid #333;border-radius:8px;font-family:'Courier New',monospace;font-size:12px;word-break:break-all;color:#fff;line-height:1.5}
.empty-state{text-align:center;color:#666;padding:40px 20px;font-size:14px}
.empty-state-icon{font-size:40px;margin-bottom:10px}
@media(prefers-color-scheme:light){body{background:#000!important;color:#fff!important}.card{background:#111!important;border-color:#333!important}input,select{background:#0a0a0a!important;border-color:#333!important;color:#fff!important}}
</style></head>
<body>
...
<div id="shopTab">
<div class="card"><h2>Новый заказ</h2>
<select id="proxyType"><option value="http">HTTP прокси</option><option value="socks5">SOCKS5 прокси</option><option value="residential">Residential прокси</option><option value="datacenter">Datacenter прокси</option></select>
<input type="number" id="quantity" placeholder="Количество (1-100)" min="1" max="100" value="1">
<select id="country"></select>
<select id="duration"><option value="1 день">Срок: 1 день</option><option value="1 неделя">Срок: 1 неделя</option><option value="1 месяц">Срок: 1 месяц</option><option value="3 месяца">Срок: 3 месяца</option></select>
<div class="price-display">Стоимость: <b id="price">$0.00</b></div>
<div class="balance" id="balance">$0.00</div>
<button class="btn" onclick="createOrder()">Оформить заказ</button></div></div>
...
function createStars(){const c=document.getElementById('stars');const s=['✦','✧','⋆','·','•','◦','⋄','∗','✶'];for(let i=0;i<40;i++){const st=document.createElement('span');st.className='star';st.textContent=s[Math.floor(Math.random()*s.length)];st.style.left=Math.random()*100+'%';st.style.fontSize=(Math.random()*10+5)+'px';st.style.animationDuration=(Math.random()*8+4)+'s';st.style.animationDelay=(Math.random()*8)+'s';st.style.opacity=Math.random()*0.3+0.1;c.appendChild(st)}}
createStars();
...
async function loadData(){const[c,p]=await Promise.all([api('/api/countries'),api('/api/prices')]);countries=c.countries||[];prices=p.prices||{};document.getElementById('country').innerHTML=countries.map(c=>`<option value="${c}">${c}</option>`).join('');updatePrice();if(userId){const u=await api('/api/user/'+userId);if(u.balance!==undefined){currentBalance=u.balance;document.getElementById('balance').textContent='$'+currentBalance.toFixed(2)}}}
...
async function createOrder(){if(!userId){tg.showPopup({title:'Ошибка',message:'Откройте через Telegram бота',buttons:[{type:'ok'}]});return}
...
if(currentBalance<total){tg.showPopup({title:'Недостаточно средств',message:'Баланс: $'+currentBalance.toFixed(2)+'\nНужно: $'+total.toFixed(2),buttons:[{type:'ok'}]});return}
const r=await api('/api/create_order','POST',{user_id:userId,proxy_type:t,quantity:q,country:c,duration:d,price:total});
if(r.success){currentBalance=r.new_balance;document.getElementById('balance').textContent='$'+currentBalance.toFixed(2);tg.showPopup({title:'Заказ оформлен',message:'Заказ #'+r.order_id+'\nОжидайте обработки',buttons:[{type:'ok'}]});tg.HapticFeedback.notificationOccurred('success')}else{tg.showPopup({title:'Ошибка',message:r.error||'Ошибка',buttons:[{type:'ok'}]});tg.HapticFeedback.notificationOccurred('error')}}
async function showTab(tab){document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));event.target.classList.add('active');document.getElementById('shopTab').classList.add('hidden');document.getElementById('ordersTab').classList.add('hidden');if(tab==='shop')document.getElementById('shopTab').classList.remove('hidden');else{document.getElementById('ordersTab').classList.remove('hidden');await loadOrders()}}
async function loadOrders(){if(!userId)return;const d=await api('/api/user/'+userId+'/orders');const o=d.orders||[];const e={'pending':'⏳','in_progress':'⟳','completed':'✓','cancelled':'✕','rejected':'✕'};const st={'pending':'Ожидает','in_progress':'В работе','completed':'Выполнен','cancelled':'Отменён','rejected':'Отклонён'};
document.getElementById('ordersList').innerHTML=o.length?o.map(o=>`<div class="order-card"><div class="order-header"><span class="order-id">#${o.id}</span><span class="status status-${o.status}">${e[o.status]||''} ${st[o.status]||o.status}</span></div><div class="order-details"><span>${o.proxy_type.toUpperCase()}</span> × ${o.quantity}<br>${o.country} · ${o.duration}<br>Сумма: <span>$${o.price.toFixed(2)}</span></div>${o.status==='completed'&&o.proxy_data?`<div class="proxy-data">${o.proxy_data}</div>`:''}</div>`).join(''):'<div class="card"><div class="empty-state"><div class="empty-state-icon">📭</div>У вас пока нет заказов</div></div>'}
